...
def validar_negacao(dict_respostas):
    ...
    # Fazendo uma previsão com o modelo (exemplo)
    previsao = modelo.predict(df_novo_cliente)

    # Retornando a previsão (ou fazendo algo com ela)
    return previsao
...
st.image("img/kima_kudi_logo_white.png")  # Substitua pela logo da Kima Kudi
...
st.caption("**Aviso:** Kima Kudi é um sistema fictício para fins educacionais. A análise é feita com um modelo de machine learning. Mais detalhes podem ser encontrados [aqui no repositório do projeto](https://github.com/diascarolina/credit-scoring-streamlit).")
...
if st.button('Analisar Crédito'):  # Botão renomeado
